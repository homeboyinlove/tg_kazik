...
# Инициализация клиента для криптовалютных платежей
        ...
        if room_id == "bot_room":
            keyboard.inline_keyboard.append(
                [InlineKeyboardButton(
                    text="Игра с ботом (ставка выбирается)",
                    callback_data="join_bot_room"
                )]
            )
        else:
            players = [
                room.get("player_1_name", "—"),
                room.get("player_2_name", "—")
            ]
...
async def join_bot_room_handler(callback_query: CallbackQuery):
    ...
    await callback_query.message.answer(
        "Введите сумму ставки (от 1 доллара):"
    )
...
async def set_bot_room_stake(message: types.Message):
    ...
    if rooms["bot_room"]["player_1"] == user_id:
        ...
        if user_profile.get("balance", 0) < stake:
            await message.answer("У вас недостаточно средств для этой ставки. Пополните баланс и попробуйте снова.")
            return
        ...
        await message.answer(
            f"Ставка {stake}$ установлена. Начинаем игру!"
        )
        await play_with_bot(user_id, stake)
    else:
        await message.answer("Вы не в комнате с ботом. Выберите комнату заново.")
...
async def join_room_handler(callback_query: CallbackQuery):
    ...
    if room_id in rooms:
        ...
        stake = room["stake"]
        user_id = callback_query.from_user.id
        user_name = callback_query.from_user.username or callback_query.from_user.full_name  # Получение ника или имени
        ...
        if room["player_1"] is None:
            ...
            await callback_query.message.answer(
                f"Вы присоединились к комнате {room_id} с номиналом ставки {stake}$ как {user_name}.")
        elif room["player_2"] is None:
            room["player_2"] = user_id
            room["player_2_name"] = user_name
            await callback_query.message.answer(
                f"Вы присоединились ко второй ячейке комнаты {room_id} с номиналом ставки {stake}$ как {user_name}.")
...
async def play_with_bot(player_id, stake):
    ...
    if player_roll > bot_roll:
        ...
        await bot.send_message(
            player_id,
            f"Вы выиграли! 🎉 Ваш бросок: {player_roll}, бросок бота: {bot_roll}.\n"
            f"Вы получили {prize}$."
        )
    elif player_roll < bot_roll:
        await bot.send_message(
            player_id,
            f"Вы проиграли. 😢 Ваш бросок: {player_roll}, бросок бота: {bot_roll}.\n"
            f"Ставка {stake}$ списана."
        )
    else:
        user_data = load_data()
        user_profile = user_data.get("users", {}).get(str(player_id), {})
        ...
        await bot.send_message(
            player_id,
            f"Ничья! Ваш бросок: {player_roll}, бросок бота: {bot_roll}.\n"
            f"Ваша ставка {stake}$ возвращена."
        )
